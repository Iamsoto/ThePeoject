...
int search(INODE * inodePtr, char * directory_name, int print)
{
...
char *parse_ptn(char **ptn){
	...
	*ptn += 1 + len;
	...
	printf("parse_ptn(): changed ptn to %s\n", *ptn);
...
int my_getino(int *dev, char *pathname){
	...
	//take the directory name and child
	
	char *parent = dirname(pathname);
	
	printf("parent in my_getino(): %s\n", parent);
	...
	while (s){
		if (strcmp(s, "__getcwd") == 0) break;
		//search to see if s is in the current inode
		printf("Passing %s into search from my_getino() loop\n", s);
		ino = search(current_inode, s, 1);
		if (ino == 0){
			printf("getino(): %s does not exist\n", s);
			printf("===========================================\n");
			return 0;
		}
		int blk    = (ino - 1) / 8 + bg_inode_table;
	...
	int child_ino    = ino;//search(current_inode, child, 1);
	...
	int child_blk    = (child_ino - 1) / 8 + bg_inode_table;
	int child_offset = (child_ino - 1) % 8;
	
	MINODE *child_res = (MINODE*)iget(fd, child_ino);
	
	*dev = child_res->dev;
		
	printf("===========================================\n");

	return child_res->ino;
}
