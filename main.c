...
extern int my_lseek(char *s_fd, char *s_position);
...
extern int my_rm(char *pathname);
...
int parse(char *param){
	char *s;
	param = dirname(param);
	while (s = (char*)parse_ptn(&param)){
		printf("[%s]\n", s);
	}
}

int dr_name(char *pts){
	printf("%s\n", dirname(pts));
}

int bs_name(char *pts){
	printf("%s\n", basename(pts));
}
...
int ls_format(char * name, INODE* inode){
	...
	for (i=8; i >= 0; i--){
		...
		if (cur_bt == 0) putchar('-');
			...
			printf("%c", bts[2 - ((i + 3) % 3)]);
		}
	}

	  printf("  ");
	  printf("%.13d ",inode->i_links_count);
...
int ls2(char *path){
	...
	if (path[0] == '/'){
		...
		int ino = my_getino(&dev, full_path);
		mip_to_list = iget(dev,ino);
	}

	printf("\npermissions i_links_count i_gid i_uid i_size\n");
	int i =0;
...
int main(int argc, char *argv[ ]) 
{
	...
	char *function_names[] = {"touch", "chmod", "chown", "chgrp", "ls", "cd", "clear", "open", "mkdir", "creat", "pwd", "rmdir", "write", "cat", "parse", "link", "dr_name", "bs_name", "unlink", "cp", "close", "move", "symlink", "rm", "quit", "pfd","lseek", 0};
	int (*fptr[])() = {touch, my_chmod, chown, my_chgrp, ls2, cd, clear, laopen_file, mkdir_creat, mkdir_creat, pwd, my_rmdir, my_write, my_cat, parse, my_link, dr_name, bs_name, my_unlink, my_cp, my_close, mv, symlink, my_rm, quit, pfd, my_lseek, 0};
